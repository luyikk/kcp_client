...
struct KcpOutput(Arc<UdpSocket>);
...
impl Kcp {
    ...
    pub fn new(conv: u32, output: Arc<UdpSocket>) -> Self {
    ...
    ///
    /// `conv` represents conversation.
    pub fn new_stream(conv: u32, output: Arc<UdpSocket>) -> Self {
        Kcp::construct(conv, output, true)
    }

    fn construct(conv: u32, output: Arc<UdpSocket>, stream: bool) -> Self {
        Kcp {
            conv,
            snd_una: 0,
            snd_nxt: 0,
